...
void Sandbox::addWaterCustom(GLContextData& contextData) const {
    ...
    Point center = Point(0.116394, 4.19664, -63.312);
    double radius = 10;
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        glVertex(center + x * (Math::cos(angle) * radius * 0.75) + y * (Math::sin(angle) * radius * 0.75));
    }
    glEnd();
...
controlPipeFd(-1) {
    ...
    if (!controlPipeName.empty()) {
        ...
        if(useSocketForPipe){
            ...
            controlSocket = new ControlSocket(controlPipeName);
            //not sure if this lambda is necessary. but hey, it works.
            handleCommand_function handler = [this] (std::string * command, handleString_function callback) {
                this->handleControlCommand(command, callback);
            };
            
            controlSocket->setCommandHandler(handler);
...
//        std::cout << "Have normalized water level: " << normalizedWaterlevel << std::endl;
        ...
        for(GLsizei count = gridSize[1] * gridSize[0]-1; count>0;--count, ++bbPtr){ 
            if(elevMin>*bbPtr)
                elevMin=*bbPtr;
            if(elevMax<*bbPtr)
                elevMax=*bbPtr;
            
            float waterLevel = *bbPtr;
            sprintf(format, "%.4g", waterLevel );
            waterlevelCallback(new std::string(format));
            waterlevelCallback(new std::string("\n")); 
		}
        */
