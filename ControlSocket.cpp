...
void ControlSocket::handleBuffer() {
    std::string cmd;

    while (std::getline(this->buffer, cmd)) {

        std::cout << "Command: [" << cmd << "]" << std::endl;

        this->handleControlCommand(new std::string(cmd));

    }


//    std::cout << "Done with the buffer." << std::endl;
}
...
void ControlSocket::handleClientEvent(epoll_event event) {
    ...
    if (eventType & EPOLLIN) {
        ...
        if ((received = recv(fd, buffer, sizeof (buffer), 0)) < 0) {
            ...
            std::cout << "Reading: [" << buffer << "] (" << received << ") into buffer" << std::endl;

//            std::cout << "Buffer is failed before: " << this->buffer.fail() << std::endl;
            this->buffer.clear();
            this->buffer << buffer;
//            std::cout << "Buffer is failed after:  " << this->buffer.fail() << std::endl;
