...
GLfloat WaterTable2::runSimulationStep(bool forceStepSize, GLContextData& contextData) const {
    ...
    if (waterColumns.size() > 0 || waterDeposit != 0.0f || !renderFunctions.empty() || !this->haveWaterlevel()) {
        /* Save OpenGL state: */
        GLfloat currentClearColor[4];
        glGetFloatv(GL_COLOR_CLEAR_VALUE, currentClearColor);
...
void WaterTable2::handleWaterColumns() const {
    int index = 0;
//    std::cout << "Handling " << waterColumns.size() << " column(s)" << std::endl;
...
//        std::cout << "Processing water column #" << ++index << " @[" << waterColumn->x << "," << waterColumn->y << ": " << waterColumn->amount << "]" << std::endl;
...
//        std::cout << "size: " << size << ", (width " << waterColumn->width << ", height " << waterColumn->height << ")" << std::endl;

        int texWidth = 0;
        int texHeight = 0;

        glGetTexLevelParameteriv(GL_TEXTURE_RECTANGLE_ARB, 0, GL_TEXTURE_WIDTH, &texWidth);
        glGetTexLevelParameteriv(GL_TEXTURE_RECTANGLE_ARB, 0, GL_TEXTURE_HEIGHT, &texHeight);

//        std::cout << "Texture: " << texWidth << "x" << texHeight << std::endl;
...
void WaterTable2::setWaterColumn(const WaterColumn* waterColumn) {
    ...
    std::cout << "WaterTable2 " << waterColumns.size() << " columns" << std::endl;
}
...
void WaterTable2::removeWaterColumn(unsigned int x, unsigned int y) {
    ...
    std::cout << "WaterTable2 " << waterColumns.size() << " columns left" << std::endl;
}

void WaterTable2::resetColumns() {
    std::cout << "WaterTable2::resetColumns()" << std::endl;
    waterColumns.clear();
    std::cout << "Water column count: " << waterColumns.size() << std::endl;
}

void WaterTable2::drain() {
